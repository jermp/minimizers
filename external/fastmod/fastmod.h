...
FASTMOD_API uint64_t mul128_u32(uint64_t lowbits, uint32_t d) { return __umulh(lowbits, d); }
...
// M = ceil( (1<<64) / d ), d > 0
FASTMOD_API uint64_t computeM_u32(uint32_t d) { return UINT64_C(0xFFFFFFFFFFFFFFFF) / d + 1; }
...
FASTMOD_API uint32_t fastdiv_u32(uint32_t a, uint64_t M) { return (uint32_t)(mul128_u32(M, a)); }
